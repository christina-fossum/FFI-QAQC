...
# 3. Fuel Constant = "Ponderosa pine"
errors_FuelsFine <- bind_rows(errors_FuelsFine,
  check_qaqc(
    fuelsfine_df,
    "Fuel Fine FuelConst",
    "Fuel Constant",
    fuelsfine_df$FWDFuConSt == "Ponderosa pine"
  )
)
...
4. Species is apropriate
5. Outlier values flagged for DBH, height, CBH
6. Correct Crown Class 
7. Correct damage code
8. Correct Plot Area
9. Correct Snag Plot Area
10. Correct Break Pt. Diameter
...
values_check <- case_when(
  is.na(trees_df$DBH) | is.na(trees_df$Subplot.Frac.) ~ FALSE,
  trees_df$DBH >= 15 & trees_df$Subplot.Frac. == 1 ~ TRUE,
  trees_df$DBH < 15 & trees_df$Subplot.Frac. == 2 ~ TRUE,
  TRUE ~ FALSE
)
values_data <- paste0("DBH: ", trees_df$DBH, ", Subplot: ", trees_df$Subplot.Frac.)
errors_Trees <- rbind(errors_Trees,
  qaqc(
    trees_df,
    "Tree Subplot Logic",
    "DBH/Subplot mismatch",
    values_check,
    values_data = values_data
  )
)
...
values_check <- values_data == "PIPO"
errors_Trees <- rbind(errors_Trees,
  qaqc(
    trees_df,
    "Tree Species",
    "Species must be PIPO",
    values_check,
    values_data = values_data
  )
)
...
# 4b. Species = PIPO
#values_data <- trees_df$Species
#values_check <- values_data %in% c("PICO", "PIEN", "ABLA")
#errors_Trees <- rbind(errors_Trees,
  #qaqc(
    #trees_df,
    #"Tree Species",
    #"Species must be one of list",
    #values_check,
    #values_data = values_data
  #)
#)
...
# 9. Outlier detection for DBH, Char Height, Scorch Height, Scorch %, Height
...
values_check <- values_data == "L"
errors_seedlings <- bind_rows(errors_seedlings, qaqc(
  data = seedling_df,
  query = "Seedling Status",
  query_message = "Status must be 'L'",
  values_check = values_check,
  values_data = values_data
))
...
values_check <- values_data %in% 0:60
errors_shrubs <- bind_rows(errors_shrubs, qaqc(
  shrub_df, "Shrub Count", "Count must be 0–60", values_check, values_data
))

# 11. Species = Approved codes
